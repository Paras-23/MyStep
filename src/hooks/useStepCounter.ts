...
// ── Re-export the task name from the single source of truth ───────────────
...
// ── Helpers (exported so backgroundStepTask.ts can import them) ────────────
...
// Separated from defineTask — registration can live in the hook but the
// defineTask MUST be in backgroundStepTask.ts (module-level, loaded at boot).
async function ensureBackgroundTaskRegistered() {
  try {
    ...
    if (!isRegistered) {
      ...
      console.log('[StepCounter] Background task registered');
    }
  } catch (err) {
    console.warn('[StepCounter] Background task registration failed:', err);
  }
}
...
export function useStepCounter() {
  ...
  const initSteps = useCallback(async () => {
    ...
    // Always take the higher value — the background task may have written a
    // higher count to Supabase while the app was closed.
  ...
  useEffect(() => {
    const sub = AppState.addEventListener('change', async (state: AppStateStatus) => {
      if (state === 'active') {
        // App came to foreground — pull fresh hardware + DB counts.
        // This is the safety net that reconciles any steps the background
        // task counted while the app was closed.
        await initSteps();
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (mounted) {
        ...
        // Register the task (defineTask lives in backgroundStepTask.ts)
